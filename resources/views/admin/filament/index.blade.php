...
@section('content')
	...
	@if(auth()->guard('web')->user()->isSuperUser() || auth()->guard('web')->user()->can('create-filaments'))
		{!! BootForm::open()->action(route('filament.index'))->post() !!}
            ...
            {!! BootForm::text('Cost per Gram', 'cost_per_gram' )->hideLabel()->helpBlock('This should be the amount the filament costs per gram for you to buy. Should be in cents.') !!}
            {!! BootForm::text('Additional Cost per Gram', 'add_cost_per_gram')->hideLabel()->helpBlock('This is any additional fee you want to charge on top of the cost per per gram. Should be in cents.') !!}
            {!! BootForm::text('Multiplier', 'multiplier')->hideLabel()->helpBlock('This value is multiplied by the cost and then weight to calculate cost.') !!}
